...
export async function createCans(container){
 ...
 scene.add(new THREE.HemisphereLight(0xffffff,0xdde5e0,.65));const key=new THREE.DirectionalLight(0xfff8ee,2.4);key.position.set(-3,5,5);scene.add(key);
 ...
 const metal=new THREE.MeshStandardMaterial({color:0xc9d4d0,metalness:.91,roughness:.28}),darkMetal=new THREE.MeshStandardMaterial({color:0x8b9997,metalness:.85,roughness:.4}),purple=new THREE.MeshStandardMaterial({color:0x59467b,metalness:.48,roughness:.28});
 ...
 for(let i=0;i<3;i++){
  ...
  const mat=new THREE.MeshStandardMaterial({map:tex,metalness:.05,roughness:.38,envMapIntensity:.55});g.add(new THREE.Mesh(bodyGeometry,mat));
