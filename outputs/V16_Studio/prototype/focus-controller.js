...
export function doorPose(p,mobile,aspect){
 const span=4.8*(mobile?1.92:1.58),center=smooth(.10,.29,p),depth=smooth(.22,.54,p);
 return {x:(mobile?0:span*aspect*.125)*(1-center),y:(mobile?-.65:-.096)*(1-center)+.42*center,z:-32*depth,scale:1-.66*depth,rotation:(mobile?.13:.22)+.7*depth,opacity:1-smooth(.40,.56,p)};
}
export async function createFocusController({container,stage,getState,wake,onFirstGrab}){
 ...
 function resize(){release();const w=stage.clientWidth,h=stage.clientHeight;aspect=w/h;camera.aspect=aspect;const span=4.8*(getState().ratio==='9x16'?1.92:1.58);camera.position.set(0,0,span/(2*Math.tan(THREE.MathUtils.degToRad(37/2))));camera.lookAt(0,0,0);camera.updateProjectionMatrix();renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.setSize(w,h);wake();}
 ...
 resize();if(!getState().staticMode)sim.push(.5);
